...
CREATE TABLE heydealer_records_new (
  ...
  customer_type TEXT NOT NULL CHECK(customer_type IN ('개인','비사업용','간이과세자','법인')),
...
SELECT id,manager,record_date,model_year,plate,model,color,
  CASE WHEN customer_type='법인(비사업용)' THEN '비사업용' ELSE customer_type END,
