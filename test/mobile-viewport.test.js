...
test('모바일 하단은 로고를 숨기고 사업자 정보를 한 행에 표시한다',()=>{
  assert.match(css,/@media\(max-width:800px\)\{footer\{[^}]*grid-template-columns:1fr[^}]*\}\.footer-brand\{display:none!important\}\.company-info\{[^}]*display:flex;flex-wrap:nowrap[^}]*white-space:nowrap\}\.company-info span\{flex:none\}/);
});
