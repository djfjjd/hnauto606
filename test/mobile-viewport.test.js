...
test('모든 주차 도면 Cell은 가독성 크기로 표시한다',()=>{
  assert.match(css,/grid-template-columns:29px repeat\(var\(--map-columns\),54px\)/);
...
test('전체 보기에서 B3·B5 2줄과 13층 1줄을 한 행 묶음으로 배치한다',()=>{
  ...
  assert.match(css,/data-map-zone="auto13"\]\{grid-column:4\/span 3;grid-row:2\/span 2\}/);
  assert.match(css,/data-map-zone="tower"\]\{grid-column:1\/span 3;grid-row:4\}/);
});
