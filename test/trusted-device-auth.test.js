...
const adminGuard=readFileSync(new URL('../functions/admin/[[path]].js',import.meta.url),'utf8');
...
test('관리자 페이지는 ADMIN_EMAIL과 Cloudflare Access 인증을 모두 요구한다',()=>{
  ...
  assert.match(adminGuard,/env\.ADMIN_EMAIL/);
  assert.match(adminGuard,/CF-Access-Authenticated-User-Email/);
  assert.match(adminGuard,/CF-Access-Jwt-Assertion/);
  assert.match(adminGuard,/email!==configured/);
