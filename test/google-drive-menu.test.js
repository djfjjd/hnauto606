...
test('상단 매입정보 메뉴에 헤이딜러와 렌터카 하위 메뉴를 함께 표시한다',()=>{
  ...
  assert.match(css,/\.parking-calendar\{position:absolute;left:50%;top:12px;min-height:40px;padding:7px 14px;[^}]*font-size:17px/);
...
test('특이사항은 확인중을 포함한 다섯 가지 필수 선택이다',()=>{
  ...
  assert.match(main,/CUSTOMER_TYPES=\['개인','비사업용','업무용으로 비용처리함','간이사업자','법인','확인중'\]/);
  assert.match(main,/value==='확인중'\?'---확인중---':value/);
  assert.match(main,/<option value="" selected disabled hidden>선택해 주세요<\/option>/);
  assert.match(main,/value==='확인중'\?'is-pending':value==='개인'\?'is-personal':'is-business'/);
...
test('선택차량목록은 기본 접힘·10개 페이지·삭제 기능을 제공한다',()=>{
  ...
  assert.match(handler,/checked_in_completed/);
  assert.match(handler,/\?='transit' AND NOT/);
