...
test('상단 헤이딜러 메뉴는 프롬프트·목록만 표시하고 Drive 아이콘은 밖에 둔다',()=>{
  ...
  assert.match(css,/\.external-tools>a:not\(\.drive-icon-link\),\.external-tools>\.external-menu>button\{font-size:14px\}/);
  assert.match(css,/\.drive-shortcut img\{width:30px;height:auto;aspect-ratio:460\/410;border-radius:50%;object-fit:cover/);
