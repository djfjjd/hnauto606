...
export function recurringMoveLabel(month,rule){
  const [adjusted,original]=recurringOccurrences(month,rule);
  return original?`(${adjusted.date.slice(-2)}일로 이동)`:'';
}
...
export function installRecurringCalendarUI({api,esc}){
  ...
  const render=()=>{grid.querySelectorAll('.calendar-day-recurring').forEach(node=>node.remove());const month=document.querySelector('[data-calendar-month]')?.textContent.match(/(\d{4}).*?(\d{2})/)?.slice(1).join('-');if(!month)return;for(const rule of rules){for(const sourceMonth of [month,nextCalendarMonth(month)]){for(const occurrence of recurringOccurrences(sourceMonth,rule)){const date=occurrence.date;if(!date.startsWith(month))continue;const day=grid.querySelector(`.calendar-day time[datetime="${date}"]`)?.closest('.calendar-day');if(!day)continue;let list=day.querySelector('.calendar-day-recurring');if(!list){list=document.createElement('div');list.className='calendar-day-recurring';day.append(list);}const button=document.createElement('button'),label=document.createElement('span');button.type='button';button.className=`calendar-recurring-item${occurrence.original?' is-original':''}`;label.textContent=rule.content;button.append(label);if(occurrence.original){const move=document.createElement('small');move.textContent=recurringMoveLabel(sourceMonth,rule);button.append(move);}button.title=`${occurrence.original?'원래 날짜':'실제 일정'} · ${rule.content}`;button.onclick=()=>openDialog(rule);list.append(button);}}}applyFilter();};
