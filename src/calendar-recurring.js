...
export function installRecurringCalendarUI({api,esc}){
  ...
  const openDialog=(rule=null)=>{
    ...
    backdrop.innerHTML=`<section class="modal calendar-recurring-modal" role="dialog" aria-modal="true" aria-labelledby="recurring-title"><button type="button" class="modal-close" data-recurring-close aria-label="닫기">×</button><h2 id="recurring-title">${rule?'반복일정 수정':'반복일정 추가'}</h2><form data-recurring-form><span class="recurring-field-title">반복 방식</span><div class="recurring-segments" role="group" aria-label="반복 방식"><button type="button" data-recurring-kind="monthly-day" aria-pressed="${kind==='monthly-day'}">매월 00일</button><button type="button" data-recurring-kind="monthly-weekday" aria-pressed="${kind==='monthly-weekday'}">매월 00째 주 00요일</button></div><div class="recurring-rule-fields"><span>매월</span><label data-recurring-week>주차<select name="weekOrdinal">${[1,2,3,4,5,0].map(value=>`<option value="${value}" ${Number(rule?.week_ordinal??(rule?.kind==='last-friday'?0:1))===value?'selected':''}>${value||'마지막'}째 주</option>`).join('')}</select></label><label data-recurring-weekday>요일<select name="weekday">${['일','월','화','수','목','금','토'].map((name,index)=>`<option value="${index}" ${Number(rule?.weekday??5)===index?'selected':''}>${name}요일</option>`).join('')}</select></label><label data-recurring-day>날짜<input type="number" name="day" min="1" max="31" value="${rule?.day||1}" required></label></div><label class="recurring-exclude"><input type="checkbox" name="excludeHolidays" ${rule?.exclude_holidays?'checked':''}>주말 및 공휴일 제외 (전 영업일로 당기기)</label><label>내용<input name="content" maxlength="500" value="${esc(rule?.content||'')}" required></label><p role="status"></p><div class="calendar-recurring-actions">${rule?'<button type="button" class="danger" data-recurring-delete>삭제</button>':''}<button type="submit" class="primary">저장</button></div></form></section>`;
