...
function parkingCell(code,spot,visible,column,visualRow,columnSpan=1,rowSpan=1){
  const position=`grid-column:${column+1}/span ${columnSpan};grid-row:${visualRow+1}/span ${rowSpan}`;
...
function blockedCell(code,column,visualRow){
  return`<div class="parking-cell is-layout-blocked" style="grid-column:${column+1};grid-row:${visualRow+1}" role="gridcell" aria-label="${code} 비주차 구역"><small>${code}</small></div>`;
}
...
export function renderParkingMap(layout,spots,visibleIds=new Set(spots.map(spot=>spot.id)),options={}){
  ...
  const collapsed=Boolean(layout.collapseBeforeRow&&!options.expanded),visibleRows=Array.from({length:layout.rows},(_,index)=>index+1).filter(row=>!collapsed||row>=layout.collapseBeforeRow),visualRowByActual=new Map(visibleRows.map((row,index)=>[row,index+1])),cells=[];
  ...
  for(const row of visibleRows){
    const visualRow=visualRowByActual.get(row);
    cells.push(`<b class="map-row" style="grid-column:1;grid-row:${visualRow+1}" aria-hidden="true">${String(row).padStart(2,'0')}</b>`);
    for(let column=1;column<=layout.columns;column+=1){
      ...
      if(match){
        ...
        const areaVisualRow=visualRowByActual.get(match.bounds.row);
        if(!areaVisualRow)continue;
        if(match.area.type==='parking'){
          const spot=byPosition.get(normalizePosition(match.area.from));
          cells.push(parkingCell(code,spot,!spot||visibleIds.has(spot.id),column,areaVisualRow,match.bounds.columnSpan,match.bounds.rowSpan));
        }else{
          cells.push(`<div class="parking-special type-${escapeHtml(match.area.type)}" style="grid-column:${column+1}/span ${match.bounds.columnSpan};grid-row:${areaVisualRow+1}/span ${match.bounds.rowSpan}" role="gridcell"><strong>${escapeHtml(match.area.label)}</strong>${match.area.label?`<small>${escapeHtml(match.area.type)}</small>`:''}</div>`);
        }
        continue;
      }
      const spot=byPosition.get(code),parking=layout.defaultCellType==='parking'||positionInRanges(code,layout.parkingRanges);
      cells.push(parking?parkingCell(code,spot,!spot||visibleIds.has(spot.id),column,visualRow):blockedCell(code,column,visualRow));
    }
  }
  const toggle=layout.collapseBeforeRow?`<button class="map-row-toggle" data-toggle-map="${escapeHtml(options.zoneId||'')}" aria-expanded="${options.expanded?'true':'false'}">${options.expanded?`01~${String(layout.collapseBeforeRow-1).padStart(2,'0')}행 숨기기`:`01~${String(layout.collapseBeforeRow-1).padStart(2,'0')}행 펼치기`}</button>`:'';
  return`<section class="parking-map" aria-label="${escapeHtml(layout.name)} 주차장 배치"><div class="parking-map-head"><div><span>9 × ${layout.rows} GRID</span><h2>${escapeHtml(layout.name)}</h2></div><div class="parking-map-actions">${toggle}<p>차량번호 뒤 4자리 표시</p></div></div><div class="parking-map-scroll"><div class="parking-map-grid" role="grid" style="--map-columns:${layout.columns};--map-rows:${visibleRows.length}">${cells.join('')}</div></div></section>`;
}
