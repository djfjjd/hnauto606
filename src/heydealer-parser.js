...
const normalizeColor=value=>{const color=clean(value);if(/블랙|나이트\s*검정색/i.test(color))return'검정';if(/화이트/i.test(color))return'흰색';if(/블루/i.test(color))return'블루';if(/은색|실버|그레이/i.test(color))return'쥐색';return color;};
